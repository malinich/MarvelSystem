    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            for keyword in keywords:
                Comixes = Comixes.filter(Q(Q(name__icontains=keyword) | Q(description__icontains=keyword) | Q(
                    tags__name__icontains=keyword))).distinct()
            if year != '0':
                print('111')
            ...
            separator = "\|/"
            output = (JSONRenderer().render(serializer.data) + separator.encode('ascii') for serializer in
                      SerializerList)
...
class Master(DetailView):
    def get(self, request, *args, **kwargs):
        ...
        if request.user.is_authenticated():
            comics = models.Comix.objects.none()
